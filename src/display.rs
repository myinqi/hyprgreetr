...
impl Display {
    ...
    fn show_horizontal_layout(&self, image_path: &std::path::PathBuf, info_lines: &[String]) -> Result<()> {
        ...
        let border_color = &self.config.display.border_color;
    ...
    fn get_module_display_name(&self, module_key: &str, default_name: &str) -> String {
        ...
        let custom_name = match module_key {
            ...
            "terminal_shell_combined" => &display_names.terminal_shell_combined,
    ...
    fn prepare_system_info_lines(&self, system_info: &SystemInfo) -> Vec<String> {
        ...
        let module_order = [
            ...
            ("terminal_shell_combined", "Terminal & Shell", modules.terminal_shell_combined),            
...
impl Display {
    fn render_border(&self, border_str: &str, hex_color: &str) -> String {
        // Use the same hex color logic as apply_color
        if let Some(rgb) = self.parse_hex_color(hex_color) {
            border_str.truecolor(rgb.0, rgb.1, rgb.2).to_string()
        } else {
            // If hex parsing fails, return uncolored text
            border_str.to_string()
        }
    }
    
    fn apply_color(&self, text: &str, hex_color: &str) -> ColoredString {
        let colored = text.normal();
        
        // Parse hex color and apply it
        if let Some(rgb) = self.parse_hex_color(hex_color) {
            colored.truecolor(rgb.0, rgb.1, rgb.2)
        } else {
            // If hex parsing fails, return uncolored text
            colored
        }
    ...
    fn parse_hex_color(&self, hex_color: &str) -> Option<(u8, u8, u8)> {
